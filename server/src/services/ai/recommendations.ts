...
export interface CostData {
  ...
  serviceCosts: { service: string; cost: number }[];
  totalMonthly: number;
}
...
const recommendationTools: OpenAI.Chat.Completions.ChatCompletionTool[] = [
  {
    type: 'function',
    function: {
...
For each optimization opportunity, call the generate_recommendation function with complete details. Generate 3-5 recommendations.`;
    ...
    if (message?.tool_calls && message.tool_calls.length > 0) {
      for (const toolCall of message.tool_calls) {
        ...
        const funcName = (toolCall as any).function?.name || (toolCall as any).name;
        const funcArgs = (toolCall as any).function?.arguments || (toolCall as any).arguments;
...
function calculateTrend(dailyCosts: { date: string; cost: number }[]): string {
...
function tryParseRecommendations(content: string): AIRecommendation[] {
  try {
...
export async function getEnhancedRecommendations(
  staticRecommendations: AIRecommendation[],
  resources: ResourceData[],
  costData: CostData[]
): Promise<AIRecommendation[]> {
  ...
  for (const aiRec of aiRecommendations) {
    const exists = allRecommendations.some(
      r => r.resourceId === aiRec.resourceId && r.type === aiRec.type
    );
